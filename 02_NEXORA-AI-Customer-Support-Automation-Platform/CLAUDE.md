# CLAUDE.md — NEXORA AI Support Copilot (MVP)

## Project Identity

**Product Name:** NEXORA AI Support Copilot
...
frontend/
  ...
  tailwind.config.ts           # NEXORA AI design tokens (brand, surface, bg, border, status colors)
...
## Design System — "NEXORA AI" (Strict)
...
EURI_TEMPERATURE=0.3
EURI_MAX_TOKENS=2048
...
This list matches `backend/app/core/config.py` and `backend/.env.example` exactly — every var above is read by `Settings`, and nothing here is unused. There is no `DATABASE_URL` or `API_BASE_URL` setting; Supabase access goes entirely through the `SUPABASE_*` vars via the Supabase client, and the backend has no notion of its own base URL.
...
- Engineering Rules: `.claude/rules/*.md` (migrated from the former `.cursor/rules/*.mdc`)
...
- This applies to ALL sessions and ALL tools (Claude Code, etc.)
