...
- **Purpose:** Named grouping of knowledge documents (e.g. "Billing FAQ", "Onboarding Docs") for organization and filtered browsing in the admin knowledge base UI.
- **Key columns:** `id`, `tenant_id`, `name`, `description`, `document_count` (denormalized counter, not auto-maintained by a trigger — the application increments/sets it), `created_at`.
- **Indexes:** `tenant_id`, `created_at`.
- **Migration:** `backend/supabase/migrations/002_knowledge_collections.sql`.
