...
| 04 | **[Secure Authentication API](04_Secure-Authentication-API)** | A JWT authentication service implementing the parts most tutorials skip — refresh-token rotation with reuse-rejection, server-side-only RBAC (no client-side self-assigned roles), and a documented token-transport security decision (access token in the response body, refresh token in an HttpOnly cookie). Built with FastAPI and React, test-first (101 automated tests), and verified live via a Dockerized PostgreSQL deployment. |
